...
v2ray_install() {
    ...
    mkdir -p /root/v2ray
    cd /root/v2ray || exit
    wget -N --no-check-certificate https://raw.githubusercontent.com/pcfreak/wago-v2ray/${github_branch}/v2ray.sh
...
nginx_exist_check() {
    if [[ -f "/etc/nginx/sbin/nginx" ]]; then
        ...
        echo -e "${OK} ${GreenBG} detects Nginx installed by other packages. If you continue to install it, it will cause conflicts. Please install ${Font} after handling . ${Font}"
        exit 1
    else
        nginx_install
    fi
...
nginx_install() {
    ...
    # Add configuration folder, adapt to the old script
...
domain_check() {
    read -rp "Please enter your domain name information(eg:www.pcfreak.com):" domain
    ...
    if [[ $(echo "${local_ip}" | tr '.' '+' | bc) -eq $(echo "${domain_ip}" | tr '.' '+' | bc) ]]; then
        ...
        echo -e "${Error} ${RedBG} Please make sure that the correct A record is added to the domain name, otherwise V2ray cannot be used normally V2ray ${Font}"
...
port_exist_check() {
    if [[ 0 -eq $(lsof -i:"$1" | grep -i -c "listen") ]]; then
        ...
        echo -e "${Error} ${RedBG} detects that port $1 is occupied, the following is the occupation information of port ${Font}"
        lsof -i:"$1"
...
acme() {
    if "$HOME"/.acme.sh/acme.sh --issue -d "${domain}" --standalone -k ec-256 --force --test; then
        echo -e "${OK} ${GreenBG} SSL certificate test issuance is successful, start to officially issue ${Font}"
...
v2ray_conf_add_tls() {
    cd /etc/v2ray || exit
    wget --no-check-certificate https://raw.githubusercontent.com/pcfreak/wago-v2ray/${github_branch}/tls/config.json -O config.json
    modify_path
    modify_alterid
    modify_inbound_port
    modify_UUID
...
v2ray_conf_add_h2() {
    cd /etc/v2ray || exit
    wget --no-check-certificate https://raw.githubusercontent.com/pcfreak/wago-v2ray/${github_branch}/http2/config.json -O config.json
    modify_path
    modify_alterid
    modify_inbound_port
    modify_UUID
...
#}
acme_cron_update() {
    wget -N -P /usr/bin --no-check-certificate "https://raw.githubusercontent.com/pcfreak/wago-v2ray/dev/ssl_update.sh"
...
tls_type() {
    if [[ -f "/etc/nginx/sbin/nginx" ]] && [[ -f "$nginx_conf" ]] && [[ "$shell_mode" == "ws" ]]; then
        ...
        echo "Please note, if you use Quantaumlt X / router / old version of Shadowrocket / V2ray core version lower than 4.18.1, please select compatibility mode"
        ...
        echo -e "${Error} ${RedBG} after or Nginx configuration file does not exist or is currently installed version h2, install the correct execution of the script{Font}"
    fi
}
show_access_log() {
...
uninstall_all() {
    ...
    if [[ -d $nginx_dir ]]; then
        ...
        [yY][eE][sS] | [yY])
            rm -rf $nginx_dir
            echo -e "${OK} ${Green} Do you want to uninstall Nginx ${Font}"
            ;;
        *) ;;
...
update_sh() {
    ol_version=$(curl -L -s https://raw.githubusercontent.com/pcfreak/wago-v2ray/${github_branch}/install.sh | grep "shell_version=" | head -1 | awk -F '=|"' '{print $3}')
    ...
    if [[ "$shell_version" < "$(sort -rV $version_cmp | head -1)" ]]; then
        ...
        [yY][eE][sS] | [yY])
            wget -N --no-check-certificate https://raw.githubusercontent.com/pcfreak/wago-v2ray/${github_branch}/install.sh
...
menu() {
    ...
    echo -e "\t---authored by WaGo-G---"
    echo -e "\thttps://github.com/pcfreak\n"
    ...
    2)
        ...
        ;;
    3)
        bash <(curl -L -s https://raw.githubusercontent.com/pcfreak/wago-v2ray/${github_branch}/v2ray.sh)
        ;;
    4)
